...
The hub has five sections behind a tab bar — **Home**, **Shapes**, **Learn**, **Progress** and **Badges**. The bar itself stays out of the way on Home, the actual landing page, and reappears everywhere else it's needed: the other four hub sections, and every activity, where it fills the blank space below the step panel instead of sitting unused. Every section is built from data the lesson already keeps (the `SHAPES` config, the five Learn-more sheets, and the saved progress store), so no section can drift out of step with the lesson it describes. Badges in particular are *derived* at render time rather than stored, so they can't disagree with progress.
...
- **One physical surface for both modes**: the staging area is a rendered whiteboard (`assets/whiteboard.jpg`) — folded and flat-net views now share it, rather than cross-fading between a dark "cosmic" gradient and a warm blueprint-paper one. Compressed from a 1.4MB source PNG to ~110KB via a headless-canvas JPEG re-encode (drawn to canvas, exported at quality 0.92) rather than shipped raw, since it's loaded on every activity screen. The source product photo carried 40–80px of plain white backdrop past the painted frame on every side — invisible on its own, but no CSS trick could hide it once the photo sat on the app's periwinkle background, since it's baked into the photo's own pixels: any crop generous enough to show the whole frame showed that backdrop too, reading as a picture pasted on a white card rather than a surface filling the stage. Cropped tight to the frame instead (1287×1016), with the narrow-phone stage shape (below) matched to that exact ratio so `cover` never has to zoom past the frame's own edges to fill the box.
...
- **The activity backdrop is deliberately out of focus**: unlike the hub room, the activity screen has a header, mode-tag, Cal's bubble and a controls panel all sitting directly on top of its background, so a sharp scene would compete with them the way the hub's did on mobile before it needed dimming after the fact. Rather than fix that after, the render itself (`assets/activity-bg.jpg`) was specified as a shallow-depth-of-field bokeh blur — hints of a bookshelf, a plant, a framed poster, nothing in focus — so no amount of UI on top of it needs its own scrim. Same optional-asset pattern as the room render (fixed, revealed on decode, absent asset just leaves the CSS gradient showing) and the same compression path, down to ~55KB.
- **One tab bar, shown where it earns its space**: the five-section switcher used to sit permanently under the hub. It now lives at document level instead of inside `#menu`, gated by two classes JS already tracks — `body.in-activity` and `body.hub-away` (the latter true whenever `showView()` lands anywhere but Home) — so it disappears from the actual landing page (nothing to switch away from there) and fills the blank space under an activity's step panel instead of leaving it empty. It stays visible on Shapes, Learn, Progress and Badges, since none of those has any other way back to Home; hiding it there too would have stranded a visitor mid-section. Tapping a tab while an activity is open exits straight to that hub section in one step (`showMenu(view)`) rather than returning to wherever the learner started first.
