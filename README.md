...
- **Hub and activity are different places, each themed on its own**: the hub is a classroom — periwinkle wall, daylight from the upper left, white panels, deep-navy type, a dark Challenge shelf. The activity took the same "inside the activity" concept: a lighter periwinkle chrome around the whiteboard stage, navy ink, glass mode-tag. Neither theme shares tokens with the other (`--hub-*` vs `--board-*`) so retouching one can't silently drag the other. A rendered hub room can still be dropped at `assets/room.png`; it's attached at runtime and only revealed once it decodes, so a missing asset leaves the CSS wall showing rather than a broken image.
- **The spotlight is tuned to what's under it**: the guided-attention vignette darkens everything outside the current focus, and the same alpha values read completely differently depending on what they're dimming. Tuned once against the old near-black cosmic background, those values crushed most of the whiteboard to mud the moment the opening was small (the warm-up step's opening starts tight, so this showed up immediately) — a navy tint at a fraction of the old opacity reads as "the room dimmed around a lit board" instead of "the board went dark."
...
- **The counters and the board speak the same icon language**: an outlined square for a face, a line for an edge, a filled dot for a corner — small glyphs under each top counter and repeated in a persistent "Solid parts" legend on the board itself, so the vocabulary from one is available at the other without relying on reading English. Drawn in CSS (`border`/`border-radius`/`background` on an empty span, not SVG or emoji) so they inherit `currentColor` and stay crisp at any size. The legend counts as stage furniture in Cal's own collision-avoidance system, the same as the mode tag or the step track, so his speech bubble slides clear of it instead of covering it.
