...
- **Two skies, on purpose**: the hub is a pastel sunset — lavender at the top through pink to warm gold, with soft clouds and drifting faceted crystals that echo the wireframe solids in the stage. The activity screens stay cosmic purple, dialled back from the full star field because the staging area is already a deep cosmic box and a strong field around it would swallow the stage. The hub's sunset is scoped to `#menu` and painted on a viewport-fixed layer inside it (the content column is only 560px wide, so painting it on `#menu` itself would leave the activity's purple showing down both margins), which leaves `body`'s rules untouched.
- **Ink is measured, not assumed**: because the hub gradient runs mid-lavender to pale gold, one flat "light ink on open background" rule can't work. A deep indigo was picked by computing contrast against every sampled gradient stop — it clears 4.5:1 the whole way down, where white peaks at 2.9:1 — and the build verifies it by sampling the actual rendered pixel behind each heading. Cards keep neutral page ink; only elements on the open sky opt in.
